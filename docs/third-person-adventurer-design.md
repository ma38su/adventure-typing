...
最初は女の子版を優先pilotとして制作する。turnaround、rig、歩行、カメラ、typing UIとの共存を承認してから、同じskeletonを使う別キャラクターの髪・服・配色差分へ展開する。先行して作成した中性的な子ども版は共通骨格と衣装比較の資料として残すが、正式モデル化の順番は女の子版を先とする。
...
- 年齢印象は小学校中学年前後。優先pilotは、丸みを残した簡潔な顔と顎までの短いボブで女の子と読めるようにする。長髪やスカートには頼らず、共通骨格を維持する。
- 大きな帽子、長い武器、巨大な背負い物、床まで届くマントは不採用。景観・予兆・typing overlayを遮るため。
- 輪郭は髪、短い上着、小型の斜め掛け鞄、裾を絞ったパンツ、短靴の5つで読む。輪郭線は使わず、2〜3段階の色面で分ける。
- 女の子版の髪は顎までの短いボブ＋小さな片側結びとし、肩・襟・鞄紐から離す。琥珀色の髪留めと葉形の上着留めだけを小さな識別accentにする。
