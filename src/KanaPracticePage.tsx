...
import { getFingerGuide, HOME_KEYS, KANA_COURSES, type KanaCourse } from './kanaPractice'
...
export function KanaPracticePage({ profileId, tutorial, onTutorialComplete, onBack, onStartAdventure }: { profileId: string; tutorial?: boolean; onTutorialComplete: () => void; onBack: () => void; onStartAdventure: () => void }) {
  ...
  return <main className={`kana-page kana-play-page ${result}`} onClick={() => inputRef.current?.focus()}>
    ...
    {finished ? <section className="kana-finish"><span>🎉</span><small>ROUTE CLEAR!</small><h1>{course.name}ルート クリア！</h1><p>{course.items.length}この問題を、さいごまで入力できたよ。</p><div><b>{course.items.length}<small>問題</small></b><b>{mistakes}<small>ミス</small></b></div><button aria-keyshortcuts="Space" onClick={continueAfterFinish}>{tutorial && course.unlocksAdventure ? '本編の冒険へ しゅっぱつ　▶' : 'つぎのルートをえらぶ　▶'}</button><small className="confirm-shortcut-hint">Spaceキーでも進める</small></section> : item && <section className="kana-practice-card">
      ...
      {fingerGuide && <div className="finger-guidance" style={{ '--finger-color': fingerGuide.color } as React.CSSProperties}><div className={`guide-hand ${fingerGuide.hand}`}><span>{fingerGuide.hand === 'left' ? '左手' : '右手'}</span><b>{fingerGuide.finger}</b></div><p>つぎは <kbd>{primaryNextKey.toUpperCase()}</kbd> を <strong>{fingerGuide.hand === 'left' ? '左手' : '右手'}の{fingerGuide.finger}</strong> で押そう</p></div>}
      <div className="finger-keyboard" aria-label="指使いキーボード">{['1234567890', 'qwertyuiop', 'asdfghjkl', 'zxcvbnm,./-'].map((row) => <div key={row}>{[...row].map((key) => { const guide = getFingerGuide(key); return <kbd key={key} className={`${nextKeys.includes(key) ? 'next' : ''} ${HOME_KEYS.includes(key) ? 'home' : ''}`} style={{ '--key-color': guide?.color } as React.CSSProperties}>{key.toUpperCase()}{HOME_KEYS.includes(key) && <i />}</kbd> })}</div>)}</div>
