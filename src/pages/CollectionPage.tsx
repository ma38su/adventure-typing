...
export function CollectionPage({ collection, points, tab, setTab, onBack }: { collection: CollectionRecord[]; points: number; tab: 'treasure' | 'friend'; setTab: Dispatch<SetStateAction<'treasure' | 'friend'>>; onBack: () => void }) {
  ...
  return <main className="collection-page">
    ...
    <section className="collection-hero"><div><span>COLLECTION BOOK</span><h1>お宝と いきものの図鑑</h1><p>同じものを見つけても数が増えるよ。コース限定のレアな出会いも探してみよう！</p></div><div className="collection-lifetime"><small>生涯のがんばり</small><b>{points.toLocaleString()}</b><span>GP</span></div></section>
    ...
    <div className="collection-grid">{rewards.map((reward) => {
      ...
      const locationHint = reward.grade && reward.course ? `${reward.grade}年生・コース${reward.course}「${COURSE_THEMES[reward.course].habitat}」` : habitats.map((habitat) => `コース${Object.values(COURSE_THEMES).findIndex((theme) => theme.habitat === habitat) + 1}「${habitat}」`).join('・')
