...
function ProfileSettingsEditor({ profile, onSave }: { profile: UserProfile; onSave: (settings: Pick<UserProfile, 'name' | 'lastGrade' | 'characterStyle'>) => string }) {
  ...
  useEffect(() => { setName(profile.name); setGrade(profile.lastGrade); setCharacter(profile.characterStyle); setMessage('') }, [profile])
  const grades: Grade[] = [1, 2, 3, 4, 5, 6]
  return <div className="profile-create-card profile-settings-card"><span>プレイ中の冒険者</span><h2>設定を変更</h2><label><span>なまえ</span><input value={name} maxLength={12} onChange={(event) => setName(event.target.value)} /></label><div className="profile-grade-picker"><span>学年</span><div>{grades.map((value) => <button key={value} className={grade === value ? 'active' : ''} onClick={() => setGrade(value)}>{value}年</button>)}</div></div><div className="profile-character-picker"><span>冒険者</span><div><button className={character === 'girl' ? 'active' : ''} onClick={() => setCharacter('girl')}><span><Explorer variant="girl" /></span><b>ミナ</b></button><button className={character === 'boy' ? 'active' : ''} onClick={() => setCharacter('boy')}><span><Explorer variant="boy" /></span><b>ソラ</b></button></div></div>{message && <p className={message === '保存しました' ? 'profile-save-success' : 'profile-error'}>{message}</p>}<button className="profile-register-button" onClick={() => setMessage(onSave({ name, lastGrade: grade, characterStyle: character }))}>設定を保存</button><small>変更内容とプレイ履歴は自動で保存されます。</small></div>
}
