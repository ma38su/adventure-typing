...
export function AudioControls({ bgmOn, soundEffectsOn, className = '', onBgmChange, onSoundEffectsChange }: AudioControlsProps) {
  return <div className={`audio-controls ${className}`.trim()} role="group" aria-label="音の設定">
    ...
    <button type="button" className="audio-toggle" aria-label={`効果音を${soundEffectsOn ? 'オフ' : 'オン'}にする`} aria-pressed={soundEffectsOn} onClick={(event) => { event.stopPropagation(); onSoundEffectsChange(!soundEffectsOn) }}><span aria-hidden="true">♪</span><b><UIRuby>効果音</UIRuby></b><small>{soundEffectsOn ? 'ON' : 'OFF'}</small></button>
  </div>
}
